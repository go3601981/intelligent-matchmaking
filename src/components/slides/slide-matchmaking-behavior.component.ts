...
@Component({
  ...
  template: `
    <app-slide-layout title="Intelligent Matchmaking" subtitle="Turning Data into Meaningful Connections">
        ...
        <div class="space-y-4">
          ...
          <div class="space-y-2">
            @for (behavior of behaviors; track behavior.text) {
              <div class="flex items-center gap-2.5 p-2.5 bg-white rounded-xl border border-gray-100 shadow-sm hover:border-blue-200 transition-colors group">
                ...
                <p class="text-gray-700 font-medium text-xs">{{ behavior.text }}</p>
              </div>
            }
          </div>
        </div>
        ...
        <div class="space-y-4">
          ...
          <div class="grid grid-cols-1 gap-2">
            @for (suggestion of suggestions; track suggestion.title) {
              <div class="relative p-3 bg-gradient-to-br rounded-xl shadow-md overflow-hidden group hover:scale-[1.01] transition-transform cursor-default"
                   [ngClass]="suggestion.gradient">
                <div class="absolute top-0 right-0 w-16 h-16 bg-white/10 rounded-full blur-2xl -mr-8 -mt-8 group-hover:bg-white/20 transition-colors"></div>
                ...
                <div class="relative z-10 flex items-center gap-3">
                  ...
                  <div class="space-y-0">
                    <h4 class="text-sm font-bold text-white">{{ suggestion.title }}</h4>
                    <p class="text-white/80 text-[9px] leading-tight">{{ suggestion.description }}</p>
                  </div>
                </div>
              </div>
            }
