...
@Component({
  ...
  template: `
    <app-slide-layout title="The Long-Term Vision" subtitle="The Future of the Expo Pass Ecosystem">
      <div class="flex flex-col items-center justify-center h-full max-w-5xl mx-auto space-y-6">
        
        <div class="text-center space-y-2">
          <h3 class="text-[9px] font-bold text-blue-600 uppercase tracking-[0.3em]">The Ultimate Goal</h3>
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-3 w-full relative">
          ...
          @for (step of visionSteps; track step.title; let i = $index) {
            <div class="relative z-10 flex flex-col items-center text-center space-y-3 group">
              <div 
                class="w-14 h-14 rounded-[20px] bg-white border border-gray-100 shadow-lg flex items-center justify-center text-blue-600 group-hover:bg-blue-600 group-hover:text-white transition-all duration-500 transform group-hover:-translate-y-2"
                [style.transition-delay]="(i * 100) + 'ms'"
              >
                <mat-icon class="text-2xl h-6 w-6">{{ step.icon }}</mat-icon>
              </div>
              
              <div class="space-y-0.5">
                <h4 class="font-bold text-gray-900 text-sm">{{ step.title }}</h4>
                <p class="text-[10px] text-gray-500 leading-tight px-2">{{ step.description }}</p>
              </div>

              @if (i < visionSteps.length - 1) {
                <div class="md:hidden flex justify-center py-1">
                  <mat-icon class="text-gray-200 text-[10px] h-3 w-3">south</mat-icon>
                </div>
              }
            </div>
          }
        ...
        <div class="w-full max-w-xl p-6 bg-gray-900 rounded-[24px] text-center relative overflow-hidden shadow-2xl">
          <!-- Background Glow -->
          <div class="absolute top-0 left-1/2 -translate-x-1/2 w-full h-full bg-blue-600/10 blur-[80px]"></div>
          
          <div class="relative z-10 space-y-3">
            <p class="text-xl font-light text-white leading-relaxed">
            ...
            <div class="flex items-center justify-center gap-3">
              ...
              <span class="text-blue-400 font-mono text-[9px] tracking-widest uppercase">The Expo Pass Vision</span>
              <div class="h-px w-6 bg-blue-500/30"></div>
            </div>
          </div>
        </div>
